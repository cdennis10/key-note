...
describe('deliberate-note gate', () => {
  it('ignores silence and uncertain input', () => {
    const initial = { sounding: false, candidate: null, since: 0 }
  ...
  it('requires stability, scores once per sustain, and resets on release', () => {
    let state = { sounding: false, candidate: null as number | null, since: 0 }
    ...
    state = updateAttemptGate(state, 69, .95, 1000).state
    expect(updateAttemptGate(state, 69, .95, 1310).accepted).toBe(69)
  })
})
